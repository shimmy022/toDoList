{
  let taskList = [
    {
      content: "dokończyć kurs programowania",
      done: false,
    },
    {
      content: "zostać trenerem przygotowania motorycznego",
      done: false,
    },
    {
      content: "zwiedzić całą Europę",
      done: false,
    },
    {
      content: "zarobić jakieś sensowne pieniądze",
      done: false,
    },
  ];
  ...
  const addNewTask = (newTaskContent) => {
    taskList = [...taskList, { content: newTaskContent }];
    render();
  };
    ...
    for (const task of taskList) {
      taskListHTMLContent += `
            <li class= "taskList__task ${
              task.done && hideDoneTasks ? "taskList__task--hidden" : ""
            } js-task">
  ...
  const renderButtons = () => {
    ...
    <button class="buttons__button js-finishAll" ${
      taskList.every(({ done }) => done) ? " disabled" : ""
    }>
    Ukończ wszystkie
    </button>`;
  };
